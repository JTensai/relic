...
public class BroodSwarm : MonoBehaviour  {
	...
	void Update () {
		...
		if (lifetime < 0.0f) {
			...
			if (health != null) {
				if (DeathFX != null) {
					//EffectBase newInstance = DeathFX.GetInstance(transform.position);
					//newInstance.PlayEffect();
				}
				health.Kill (AttackEnum.Charger);
				
				if (DeathSoundEffect.SoundFile != null) {
					Debug.Log ("Playing swarm death SFX");
	...
	void KillTarget() {
		...
		if(Vector3.Distance(attackTarget.transform.position,this.gameObject.transform.position) <.7)
		{
		...
		if (onCreep) {
				mover.Move (0, (gameObject.transform.forward) * Time.deltaTime * curve * attackSpeed*1.5f);

		} 
		else {mover.Move (0, (gameObject.transform.forward) * Time.deltaTime * curve * attackSpeed);
