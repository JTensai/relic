...
public class FadingForcefieldResizeable : Forcefield {
		...
		// 
		public Vector3 size = new Vector3 (2f, 3f, 4f);
		...
		void Update () {
			if (!poweredOff) {
						if (!faded) {
								...
								meshColliderChild.transform.localScale = new Vector3 (size.x, alpha, size.z);
						} else {
							//faded = false;
	...
	//	GL.MultMatrix(rotationMatrix);    
		...
		//	Gizmos.DrawWireCube(transform.position+  new Vector3(0, size.y/2, 0), new Vector3( size.x,size.y, size.z));
			Gizmos.DrawWireCube( new Vector3(0, size.y/2, 0), new Vector3( size.x,size.y, size.z));
