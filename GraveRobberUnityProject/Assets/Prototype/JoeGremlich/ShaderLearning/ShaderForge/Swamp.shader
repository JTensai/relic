...
/////// Diffuse:
                ...
                float4 node_8558 = _Time + _TimeEditor;
                float2 node_815 = (i.uv0+node_8558.g*float2(0.0125,0.0125));
                float4 _Swamp1_var = tex2D(_Swamp1,TRANSFORM_TEX(node_815, _Swamp1));
                float2 node_1683 = (i.uv0+node_8558.g*float2(-0.0125,-0.0125));
...
////// Emissive:
                float2 node_1031 = (i.uv0+node_8558.g*float2(-0.0025,-0.005));
                float4 _Highlight1_var = tex2D(_Highlight1,TRANSFORM_TEX(node_1031, _Highlight1));
                float2 node_6939 = (i.uv0+node_8558.g*float2(0.0025,0.005));
...
/////// Diffuse:
                float NdotL = max(0.0,dot( normalDirection, lightDirection ));
                float3 directDiffuse = max( 0.0, NdotL) * attenColor;
                float4 node_5592 = _Time + _TimeEditor;
                float2 node_815 = (i.uv0+node_5592.g*float2(0.0125,0.0125));
                float4 _Swamp1_var = tex2D(_Swamp1,TRANSFORM_TEX(node_815, _Swamp1));
                float2 node_1683 = (i.uv0+node_5592.g*float2(-0.0125,-0.0125));
                float4 _Swamp2_var = tex2D(_Swamp2,TRANSFORM_TEX(node_1683, _Swamp2));
                float3 node_3673 = (_Swamp1_var.rgb*_Swamp2_var.rgb);
