...
public class LargeTileSplitterEditor : Editor {
	...
	public override void OnInspectorGUI()
	{
		...
		SerializedProperty tileCategory = serializedObject.FindProperty("tileCategory");
		tileCategory.stringValue = EditorGUILayout.TextField("Tile Category", tileCategory.stringValue);
...
//		SerializedProperty isVerticle = serializedObject.FindProperty("isVerticle");
		...
		SerializedProperty offsetX = serializedObject.FindProperty("offsetX");
		SerializedProperty offsetY = serializedObject.FindProperty("offsetY");

		offsetX.intValue = EditorGUILayout.IntField ("Offset X", offsetX.intValue);
		offsetY.intValue = EditorGUILayout.IntField ("Offset Y", offsetY.intValue);
	...
	private void SavePrefab(LargeTileSplitter splitter, Mesh renderMesh, Mesh colliderMesh, PhysicMaterial physicsMaterial, int x, int y)
	{
		...
		tile.tileGroup = splitter.tileCategory;
	...
	private void Split()
	{
		...
		if (splitter.offsetX != 0 || splitter.offsetY != 0)
		{
			renderMesh = renderMesh.MakeCopy();
			renderMesh.ShiftVertices(new Vector3(splitter.offsetX, 0.0f, splitter.offsetY));
		}
	...
	public void OnSceneGUI()
	{
		...
		for (int x = 0; x < splitter.Width; ++x)
		{
			for (int y = 0; y < splitter.Height; ++y)
			{
				DrawQuad(splitter.transform, startPosition + new Vector3(x - splitter.offsetX, 0, y - splitter.offsetY), new Color(0.1f, 0.7f, 0.3f, 0.0f));
			}
		}
	}
}
