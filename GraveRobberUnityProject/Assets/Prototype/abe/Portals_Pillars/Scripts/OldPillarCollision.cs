...
public class OldPillarCollision : MonoBehaviour 
{
	...
	{		
		...
		if(isActive)
		{
			...
			else if(other.gameObject.name.Contains ("Batch") || other.gameObject.name.Contains ("Floor") || 
			        other.gameObject.name.Contains ("Tile") || other.gameObject.name.Contains ("Default Physics"))
			{
				int myDesignation = 0;
				if(name.Contains("Bottom"))
					myDesignation = 1;
				else if(name.Contains ("MiddleLow"))
				  	myDesignation = 2;
				else if(name.Contains ("MiddleHigh"))
					myDesignation = 3;
				else if(name.Contains ("Top"))
					myDesignation = 4;
				PillarRoot.GetComponent<PillarMain>().UpdatePivot (myDesignation);
			}
		}
		...
		if(other.name.Contains("Pillar"))
		{
			...
			{
				if(other.GetComponent<OldPillarCollision>() != null && PillarName != other.GetComponent<OldPillarCollision>().PillarName)
				{
					PillarMain ToPush = other.GetComponent<OldPillarCollision>().PillarRoot.GetComponent<PillarMain>();
					...
					else if(ToPush.CurrentFallSpeed() == zero)
					{
						int myDesignation = 0;
						if(name.Contains("Bottom"))
							myDesignation = 1;
